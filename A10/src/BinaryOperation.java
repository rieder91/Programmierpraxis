...
public class BinaryOperation implements Operation {
	 ...
	 * 
	 * @param threshold
	 *            Schwellenwert für den Algorithmus
	 */
	...
	/**
	 * Ersetzt alle Zeichen vor dem Schwellenwert mit dem dunkelsten und alle
	 * danach mit dem hellsten "Zeichen"
	 * 
	 * @return binäres Bild
	 */
	public AsciiImage execute(AsciiImage img) throws OperationException {
		...
		if (limit == -1) {
			...
			for (int i = 0; i < charset.length(); i++) {
				if (i < limit) {
					op = new ReplaceOperation(charset.charAt(i),
							charset.charAt(0));
					ret = op.execute(ret);
				} else if (i >= limit) {
					op = new ReplaceOperation(charset.charAt(i),
